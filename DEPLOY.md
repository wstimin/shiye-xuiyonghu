...
1Panel 和宝塔的部署方式已经拆成独立文档，避免和一键安装教程混在一起。面板部署时推荐按对应文档操作：

- [1Panel 部署教程](./1Panel部署教程.md)
- [宝塔部署教程](./宝塔部署教程.md)

面板部署的核心流程是：先在面板创建 MySQL 数据库，再上传完整项目到 Node.js 运行目录，启动项目后通过网页安装向导绑定数据库。请特别注意运行目录根部必须能看到 `package.json`、`server.js` 和 `public/`。
